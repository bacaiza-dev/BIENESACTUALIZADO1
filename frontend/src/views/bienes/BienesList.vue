<template>
  <div class="space-y-6">
    ...
    <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-xl border border-gray-200 dark:border-gray-700 p-4 flex items-center gap-4">
      <label class="font-medium text-gray-700 dark:text-gray-300 whitespace-nowrap">
      ...
      <select
        ...
        @change="cargarBienes()"
        class="flex-1 px-4 py-2 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-white font-medium focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all"
      >
        <option value="">Todos</option>
        <option v-for="ubicacion in ubicaciones" :key="ubicacion.id" :value="String(ubicacion.id)">
          {{ ubicacion.area }}
        </option>
      </select>
    </div>
...
const cargarBienes = async () => {
  ...
  try {
    ...
    const response = await apiClient.get('/bienes', { params })
