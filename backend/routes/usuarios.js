...
// Función simplificada para validar cédula ecuatoriana
function validateCedulaEcuador(cedula) {
  ...
  const province = parseInt(cedula.substring(0, 2), 10);
  if (province < 1 || province > 24) {
    return { valid: false, error: 'Provincia inválida en cédula' };
  }
  const third = parseInt(cedula.charAt(2), 10);
  if (third >= 6) {
    return { valid: false, error: 'Tipo de cédula inválido' };
  }
  ...
  const verifier = (10 - (sum % 10)) % 10;
  if (verifier !== parseInt(cedula.charAt(9), 10)) {
    return { valid: false, error: 'Checksum inválido en cédula' };
  }
  return { valid: true };
}
...
router.post("/", verifyToken, requireAdmin, async (req, res) => {
  try {
    ...
    const { nombres, apellidos, email, password, cedula, telefono, rol_id, departamento, departamento_id, area_id, activo } = req.body;

    // Validación de cédula (Ecuador)
    if (cedula) {
      const validation = validateCedulaEcuador(cedula);
      if (!validation.valid) {
    ...
    // Usar area_id, por defecto área 1 (Desarrollo de software)
    const finalAreaId = area_id || 1;

    const result = await query(
      `INSERT INTO usuarios (nombres, apellidos, email, password_hash, cedula, telefono, rol_id, departamento_id, area_id, activo)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [nombres, apellidos || null, email.toLowerCase(), passwordHash, cedula || null, telefono || null, rol_id || null, finalDepartamentoId, finalAreaId, activo !== false ? 1 : 0]
    );
...
router.put("/:id", verifyToken, requireAdmin, async (req, res) => {
  try {
    const { id } = req.params;
    const { nombres, apellidos, email, password, cedula, telefono, activo, rol_id, departamento, departamento_id, area_id } = req.body;
    ...
    // Validar email duplicado (excluye el usuario actual)
    ...
    const params = [nombres, apellidos || null, email, cedula || null, telefono || null, activo ? 1 : 0, rol_id || null, finalDepartamentoId, finalAreaId];
